#pragma once
#include "calls.h"
#define QPC_HOOK (void*)0x7CA064

__declspec(naked) void TextureFilteringHook() {
	__asm {
		mov cl, byte ptr ds:[0x867AFC]
		test cl, cl
		je DONE
		mov cl, byte ptr ds:[0x8A6305]
		test cl, cl
		jne CHECKNEXT
		cmp eax, 1
		je RETZERO
	CHECKNEXT:
		mov cl, byte ptr ds:[0x8A6306]
		test cl, cl
		jne DONE
		cmp eax, 2
		jne DONE
...
__declspec(naked) void AntiAliasingHook() {
	__asm {
		...
		mov edx, 1
		shl edx, cl
		test dword ptr ds:[0x8A6300], edx
		jne DONE
		xor eax, eax
	DONE :
		ret
...
void writePatches() {
	WriteRelJump(0x404394, (UInt32)TextureFilteringHook);
	WriteRelJump(0x4043E4, (UInt32)TextureColorDepthHook);
	WriteRelJump(0x4044B4, (UInt32)AntiAliasingHook);
}
